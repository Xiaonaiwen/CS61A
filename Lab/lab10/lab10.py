...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        elif operator in OPERATORS: # Call expressions
            return calc_apply(OPERATORS[operator], operands.map(calc_eval)) # UPDATE THIS FOR Q2, what is type(operator)?
        else:
            return calc_eval(Pair(bindings[operator], operands))
...
def eval_and(expressions):
    ...
    if expressions is nil:
        ...
        if expressions.first is scheme_f:
            ...
            return eval_and(expressions.rest.map(calc_eval))
...
def eval_define(expressions):
    ...
    temp = expressions.rest.first
    if temp in OPERATORS:
        bindings[symbol] = temp
    else:
        bindings[symbol] = calc_eval(temp)
    return symbol
