...
def boar_brawl(player_score, opponent_score):
    ...
    remain2Digits = opponent_score % 100
    tensDigit = remain2Digits // 10
    score = abs(tensDigit-onesDigit) * 3
    if score == 0:
        return 1
    else:
        return score
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    numFactors = num_factors(score)
    newScore = score
    if numFactors == 3 or numFactors == 4:
        n = score + 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    constant_check = strategy(0,0)
    i = 0
    k = 0
    while i < goal:
        while k < goal:
            if constant_check != strategy(i,k):
                return False
            k += 1
        i += 1
        k = 0
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    largest_average = 0
    while num <= 10:
        temp = make_averaged(roll_dice,times_called)(num,dice)
        if temp > largest_average:
            largest_average = temp
            low_roll = num
        num = num + 1
    return low_roll
    # END PROBLEM 9
